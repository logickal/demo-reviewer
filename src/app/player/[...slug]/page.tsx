...
const CommentItem = ({
  ...
  return (
    <div className={`space-y-3 ${depth > 0 ? 'ml-12' : ''}`}>
      <div
        className={`group p-4 rounded-2xl border transition-all flex gap-4 items-start ${hoveredCommentTimestamp === comment.timestamp ? 'bg-orange-50 border-orange-200 shadow-md' : depth > 0 ? 'bg-gray-50/50 border-gray-100' : 'bg-white border-gray-100 hover:border-gray-200'}`}
        onMouseEnter={() => setHoveredCommentTimestamp(comment.timestamp)}
        onMouseLeave={() => setHoveredCommentTimestamp(null)}
      >
        <div className={`shrink-0 ${depth > 0 ? 'w-8 h-8 rounded-lg' : 'w-10 h-10 rounded-xl'} bg-gray-100 text-gray-500 flex items-center justify-center text-xs font-black group-hover:bg-orange-500 group-hover:text-white transition-colors`}>
          {comment.initials.substring(0, 2).toUpperCase()}
        </div>
        <div className="grow">
          <div className="flex items-center justify-between mb-1">
            <span className="text-orange-500 font-mono text-xs font-bold bg-orange-50 px-2 py-0.5 rounded-full">{formatTime(comment.timestamp)}</span>
          ...
          <p className={`${depth > 0 ? 'text-gray-600 text-xs' : 'text-gray-700 text-sm'} leading-relaxed`}>{comment.text}</p>
        </div>
      </div>
      ...
      <main className="flex min-h-screen flex-col items-center p-24 bg-gray-50">
        <div className="z-10 w-full max-w-5xl">
          <div className="mb-8 flex justify-between items-start">
            <div>
              {!isGuest && (
                <Link href={folderPath.includes('/') ? `/player/${folderPath.split('/').slice(0, -1).join('/')}` : '/'} className="text-blue-500 mb-4 hover:underline flex items-center gap-1">
    ...
    <main className="flex min-h-screen flex-col items-center p-24 bg-gray-50">
      <div className="z-10 w-full max-w-5xl">
        ...
        <div className="mb-8 p-6 border-0 rounded-3xl bg-white shadow-xl shadow-gray-200/50">
          ...
          <div className="relative w-full h-[180px] bg-gray-50 mb-6 rounded-2xl border border-gray-100" onClick={handleWaveformClick}>
            ...
            {newCommentTimestamp !== null && duration > 0 && (
              <div className="absolute z-50 bg-white p-4 rounded-2xl shadow-2xl border border-gray-100 w-72 text-sm animate-in fade-in zoom-in duration-200" style={{ left: `${Math.min(Math.max((newCommentTimestamp / duration) * 100, 10), 90)}%`, top: '50%', transform: 'translate(-50%, -50%)' }} onClick={(e) => e.stopPropagation()}>
                ...
                <input autoFocus type="text" value={newCommentInitials} onChange={(e) => setNewCommentInitials(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-orange-500 focus:ring-2 focus:ring-orange-100 outline-none transition-all mb-3 text-xs text-gray-900" placeholder="Initials (e.g. JD)" maxLength={3} />
                <textarea value={newComment} onChange={(e) => setNewComment(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-orange-500 focus:ring-2 focus:ring-orange-100 outline-none transition-all mb-4 text-xs resize-none text-gray-900" placeholder="What do you think?" rows={3} />
